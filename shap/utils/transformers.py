...
def parse_prefix_suffix_for_tokenizer(tokenizer):
    ...
    if len(null_tokens) == 1:
        ...
        assert ("eos_token" in tokenizer.special_tokens_map) or (
            "bos_token" in tokenizer.special_tokens_map
        ), "No eos token or bos token found in tokenizer!"
        if ("eos_token" in tokenizer.special_tokens_map) and (
            tokenizer.decode(null_token) == tokenizer.special_tokens_map["eos_token"]
        ):
            keep_prefix = 0
            keep_suffix = 1
            prefix_strlen = 0
            suffix_strlen = len(tokenizer.decode(null_tokens[-keep_suffix:]))
        elif ("bos_token" in tokenizer.special_tokens_map) and (
            tokenizer.decode(null_token) == tokenizer.special_tokens_map["bos_token"]
        ):
            keep_prefix = 1
            keep_suffix = 0
