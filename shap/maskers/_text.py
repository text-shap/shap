...
class Text(Masker):
    ...
    def __init__(
        ...
        if mask_token == "auto":
            if (
                ...
                self.mask_token = (
                    " " + self.tokenizer.decode([self.tokenizer.mask_token_id]) + " "
                )  # [self.prefix_strlen:-self.suffix_strlen]
                if self.keep_suffix > 0:
                    self.mask_token_id = tokenizer.encode(self.mask_token)[
                        self.keep_prefix : -self.keep_suffix
                    ]
                else:
                    self.mask_token_id = tokenizer.encode(self.mask_token)[
                        self.keep_prefix :
        ...
        if self.keep_suffix > 0:
            self.mask_token_segment = self.token_segments(self.mask_token)[
                self.keep_prefix : -self.keep_suffix
            ]
        else:
            self.mask_token_segment = self.token_segments(self.mask_token)[
    ...
    def __call__(self, mask, s):
        ...
        if self.output_type == "string":
            ...
            if safe_isinstance(
                self.tokenizer, "transformers.tokenization_utils.PreTrainedTokenizer"
            ):
        ...
        if self.output_type == "string":
            decoded_str = self.tokenizer.decode(out)[self.prefix_strlen :][
                : -self.suffix_strlen
            ].strip()
        ...
        if safe_isinstance(
            ...
            if type(s) is tuple:
                return self.tokenizer.encode_plus(
                    s[0], s[1], return_offsets_mapping=True
                )
            else:
    ...
    def token_segments(self, s):
        if safe_isinstance(
            ...
            token_ids = (
                self.tokenizer.encode_plus(s[0], s[1])["input_ids"]
                if type(s) is tuple
                else self.tokenizer.encode_plus(s)["input_ids"]
            )
            ...
            tokens = [
                tokens[i] if (special_tokens_mask[i] == 0 or type(s) is tuple) else ""
                for i in range(len(special_tokens_mask))
            ]  # TODO: correct handling of special internal tokens
            return tokens
            ...
            offsets = (
                self.tokenizer.encode_plus(s[0], s[1], return_offsets_mapping=True)[
                    "offset_mapping"
                ]
                if type(s) is tuple
                else self.tokenizer.encode_plus(s, return_offsets_mapping=True)[
                    "offset_mapping"
                ]
            )
            # TODO: handle offsets, currently this does not use s_opt which is obviously wrong
    ...
    def _mark_uninvertable(self, clustering):
        ...
        def recursive_mark(ind):
            ...
            if not np.all(e2[1:-1] == tmp):
                clustering[
                    ind - M, 2
                ] = -1  # set the distance of this cluster negative so it can't be split
    ...
    def invariants(self, s):
        ...
        for i, segment in enumerate(
            self._segments_s
        ):  # TODO: How should special characters be handled in invariants?
            if segment == "</s>":
                invariants[i] = True
...
def merge_score(group1, group2):
    ...
    if group2[0].s in [".", "?", "!"]:
        if len(group2) > 1:  # reach across
            ...
            score -= 1
    # if (
    #     group1[0].s in separators
    #     or group1[-1].s in separators
    #     or group2[0].s in separators
    #     or group2[-1].s in separators
    # ):
    #     score -= 5000
...
def partition_tree(decoded_tokens):
    ...
    for i in range(len(token_groups) - 1):
        scores = [
            merge_score(token_groups[i], token_groups[i + 1])
            for i in range(len(token_groups) - 1)
        ]
        #         print(scores)
        ind = np.argmax(scores)
