...
class TFTeacherForcingLogits(TeacherForcingLogits):
    def __init__(
        ...
        """
        super(TFTeacherForcingLogits, self).__init__(
            model,
            tokenizer,
    ...
    def get_teacher_forced_logits(self, source_sentence_ids, target_sentence_ids):
        ...
        if self.similarity_model.config.is_encoder_decoder:
            ...
            target_sentence_ids = tf.concat(
                (target_sentence_start_id, target_sentence_ids), axis=-1
            )
            ...
            combined_sentence_ids = tf.concat(
                (source_sentence_ids, target_sentence_ids), axis=-1
            )
            # generate outputs and logits
            if self.device is None:
                ...
                try:
                    with tf.device(self.device):
                        outputs = self.similarity_model(
                            combined_sentence_ids, return_dict=True
                        )
                except RuntimeError as e:
                    print(e)
            # extract only logits corresponding to target sentence ids
            logits = outputs.logits.numpy()[
                :, source_sentence_ids.shape[1] - 1 :, :
            ].astype("float64")
        del outputs
        return logits